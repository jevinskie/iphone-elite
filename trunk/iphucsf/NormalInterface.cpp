...
int n_getfile(string *args, struct shell_state *sh)
{
	...
	{
		remote_file_size = atoi(args[3].c_str());
	}
	...
	get_file(sh, (char *)args[2].c_str(), (char *)args[1].c_str(), remote_file_size);
...
#define MAX_GETFILE_BUFFER (4096)
void get_file(struct shell_state *sh, char *local_path, char *remote_path, int remote_file_size)
{
        FILE *f;
        unsigned char *buf;
        afc_file_ref ref;
        ...
        unsigned int buffer_size = remote_file_size;
        if (remote_file_size > MAX_GETFILE_BUFFER)
        {
                ifVerbose cout << "File is too large for single call. Buffering.";
                buffer_size = MAX_GETFILE_BUFFER;
        }
        ...
        unsigned int bytes_remaining = remote_file_size;
        ...
        {
                unsigned int read_size = bytes_remaining;
                if (read_size > buffer_size)
                        read_size = buffer_size;
       
                bytes_remaining -= read_size;
               
                ifNotQuiet cout << "Reading " << read_size << " bytes, " << bytes_remaining << " bytes remain" << endl;
                D("AFCFileRead");
                ret = AFCFileRefRead(sh->conn, ref, buf, &read_size);
                if (ret)
                {
